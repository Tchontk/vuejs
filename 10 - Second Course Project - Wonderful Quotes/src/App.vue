...
export default {
  ...
  created() {
    quoteBus.$on('editQuotes', (message) => { (this.quotes.length >= this.quoteMax) ? alert('No more quotes') : this.quotes.push(message) })
    quoteBus.$on('deleteQuote', (index) => { this.quotes.splice(index - 1, 1) })
  }
}
</script>
